var slideparser = (function() {
	function processSlideSource (slideSrc) {
		...
		for (var i in tokens) {
			...
			tokens[i].body = convertUrlsToAnchor(tokens[i].body);
			...
			switch (tokens[i].token) {
				...
				case "codemark":
					...
					break;
				case "diagrammark":
					slide += "<svg id='svg"+i+"'></svg>\n";
					slide += "<textarea class='diagramSource' x-svgLink='svg"+i+"'>\n";
					inDiagram = true;
					break;
				case "empty":
					slide += tokens[i].body+"\n";
	...
	function convertUrlsToAnchor(str) {
		var escaped = String(str).replace(/([^=][^"'])(https?:\/\/[^\s]+)/g, '$1<a href="$2" target="_blank">$2</a>');
		return escaped;
	}
	
	function unescapeWhitelist(str) {
		var whitelist = new Array("strong", "img", "em", "span", "p", "pre", "h1", "h2", "h3", "h4", "h5", "h6", "!--", "table", "tr", "th", "td");
